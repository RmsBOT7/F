...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo Lord @${num.split('@')[0]}\nSelamat Datang Di Group *${mdata.subject}* Semoga Betah Ya Bro Jika Belum Save Nomer Admin Silahkan Save Dengan Ketik $creator Powered :*${mdata.subject}*`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6282215689779@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["6281281872699@s.whatsapp.net"]
			...
			case 'creator':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/yta?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
               ...
               case 'infogempa':
				...
				case 'ytmp4':
					if (args.length < 1) return reply('Urlnya mana um?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/ytv?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Title* : ${anu.title}`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
